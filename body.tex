...
The next level is created by aggregating the average of a cube of 8 voxels in the lower level to one voxel in the higher level. This process is repeated until the last level, consisting of only one voxel, is reached. 
...
\begin{equation}
pyr_{range}(\vec{x}, n) = 1 \over 4 ((pyr_{max}(\vec{x}, n)_{r} - pyr_{min}(\vec{x}, n)_{r}) + (pyr_{max}(\vec{x}, n)_{g} - pyr_{min}(\vec{x}, n)_{g}) +(pyr_{max}(\vec{x}, n)_{b} - pyr_{min}(\vec{x}, n)_{b}) + (pyr_{max}(\vec{x}, n)_{\alpha} - pyr_{min}(\vec{x}, n)_{\alpha}))
\end{equation}
...
More specifically, this algorithm works by letting the user select an opacity threshold $k \in [0,1]$.
Then, the rays are cast through the maximum pyramid at the highest level. For each cell of the maximum pyramid that a ray intersects, the algorithm checks wether the opacity in that cell lies above or below k. If that value is larger than k, we move one level down. If the opacity is smaller than k it means that the region of space encompased by the cell is almost empty. Therefore, the entire region can be approximated by sampling the cell in the average pyramid that corresponds to the current cell in the maximum pyramid (meaining same position and level).
