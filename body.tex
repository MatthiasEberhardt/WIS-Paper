...
This approach could be generalized to calculate multi-scattering (light that needs two or more scattering events to reach the camera), but this would very quickly become too costly. Therefore, when rendering images with global illumination, more sophisticated algorithms are necessary[REFORMULATE! + SOURCES].
...
A common technique in classical ray tracing to generate unbiased images with global illumination so-called monte carlo ray tracing (MCRT) approach which estimates the rendering equation by stochastically sampling it (as opposed to uniform sampling used by the ray marching algorithm described above).
