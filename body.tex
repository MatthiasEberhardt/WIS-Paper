...
From the top-down view, the particles will occupy an area of ${n_sA}=\frac{{\rho}_sAE{\Delta}h}{E}$ and f ${n_aA}=\frac{{\rho}_aAE{\Delta}h}{E}$, respectively, if we assume that the particles do not overlap each other, which is reasonable if the densities and the height do not become too large.
...
\begin{equation}
 \prod_{1\le j \le i}(\tau(p_{j-1}, p_j)) \cdot \mu_a(p_{i})\epsilon (p_{i}, \omega)sp_{i}
\end{equation} 
\begin{figure}[htb]
  \centering
